...
- **iPhone Safari のメモリ・速度制限**: 大きいモデルは載らない。whisper-base 級
  (量子化 ~60MB)を第一候補に tiny / small と実機比較する
  (kotoba-whisper 系は実際 370MB 超のため候補から除外。docs/design.md §6)
- **PWA スタンドアロンでのマイク権限**: iOS 16.4 以降は動作するはずだが、権限の再プロンプト
  頻度も含めフェーズ 1 で実機検証必須。Safari タブ運用にフォールバックする場合は
  **7 日 ITP 消去のリスクを負う**(ホーム画面追加は免除)ため、エクスポート導線を強化する
- **無音・小声での幻覚出力**: Whisper は静かな音声で「ご視聴ありがとうございました」等を
  出力する既知問題がある。RMS 音量ゲート+ブラックリストで防ぐ(docs/design.md §6)
